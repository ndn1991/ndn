package com.vinecom.rec.app.ultil
...
object Utils {
  ...
  def getLong(s: String): Long = {
    try {
      ...
      case e: NumberFormatException =>
        val l = new BigInteger(s.replaceAll("-", "").replaceAll("_", ""), 16).longValue()
        if (l < 0) l
        else -l
    }
  }
  ...
  def getInt(s: String): Int = {
    try {
      ...
      case e: NumberFormatException =>
        val l = new BigInteger(s.replaceAll("-", "").replaceAll("_", ""), 16).intValue()
        if (l < 0) l
        else -l
    }
  }
